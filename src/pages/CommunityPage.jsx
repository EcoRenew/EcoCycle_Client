...
const CommunityPage = () => {
  ...
  const [newPost, setNewPost] = useState({ title: '', description: '', images: [], tags: [] });
  const [newTag, setNewTag] = useState('');
  const popularTags = ['upcycling', 'garden', 'fabric', 'wood', 'glass', 'sustainable'];
  ...
  const addComment = (postId) => {
    ...
    setPosts(posts.map(post =>
      post.id === postId
        ? { ...post, comments: [...post.comments, newCommentObj], showComments: true }
        : post
    ));
  ...
  const handleSharePost = () => {
    if (!newPost.title.trim() || !newPost.description.trim() || newPost.images.length === 0) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-4xl mx-auto px-4 py-6">
        <TagFilterBar filter={filter} setFilter={setFilter} popularTags={popularTags} />
        {searchQuery && <SearchInfoBox searchQuery={searchQuery} resultCount={filteredPosts.length} setSearchQuery={setSearchQuery} />}
